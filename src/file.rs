...
impl XvcFile {
    ...
    fn track(&self, targets: &Bound<PyTuple>, opts: Option<&Bound<PyDict>>) -> PyResult<String> {
        ...
        update_cli!(opts, &mut cli_opts, flag ["no-commit", "no_commit"] => "--no-commit");
        update_cli!(opts, &mut cli_opts, ["text-or-binary", "text_or_binary"] => "--text-or-binary");
        update_cli!(opts, &mut cli_opts, flag ["force"] => "--force");
        update_cli!(opts, &mut cli_opts, flag ["no-parallel", "no_parallel"] => "--no-parallel");
    ...
    fn hash(&self, targets: &Bound<PyTuple>, opts: Option<&Bound<PyDict>>) -> PyResult<String> {
        ...
        update_cli!(opts, &mut cli_opts, flag ["help"] => "--help");
        update_cli!(opts, &mut cli_opts, ["algorithm"] => "--algorithm");
        update_cli!(opts, &mut cli_opts, ["text-or-binary", "text_or_binary"] => "--text-or-binary");
    ...
    fn send(&self, targets: &Bound<PyTuple>, opts: Option<&Bound<PyDict>>) -> PyResult<String> {
        ...
        update_cli!(opts, &mut cli_opts, flag ["help"] => "--help");
        update_cli!(opts, &mut cli_opts, ["remote", "to", "storage"] => "--storage");
        update_cli!(opts, &mut cli_opts, flag ["force"] => "--force");
        update_targets(targets, &mut cli_opts)?;
        self.run(cli_opts)
    }
    ...
    fn remove(&self, targets: &Bound<PyTuple>, opts: Option<&Bound<PyDict>>) -> PyResult<String> {
        ...
        update_cli!(opts, &mut cli_opts, flag ["help"] => "--help");
        update_cli!(opts, &mut cli_opts, flag ["force"] => "--force");
        update_cli!(opts, &mut cli_opts, flag ["from-cache", "from_cache"] => "--from-cache");
        update_cli!(opts, &mut cli_opts, ["from-storage", "from_storage"] => "--from-storage");
        update_cli!(opts, &mut cli_opts, flag ["all_versions", "all-versions"] => "--all-versions");
        update_cli!(opts, &mut cli_opts, ["only_version", "only-version"] => "--only-version");
        update_targets(targets, &mut cli_opts)?;
        self.run(cli_opts)
    }
    ...
    fn share(&self, targets: &Bound<PyTuple>, opts: Option<&Bound<PyDict>>) -> PyResult<String> {
        ...
        update_cli!(opts, &mut cli_opts, flag ["help"] => "--help");
        update_cli!(opts, &mut cli_opts, ["remote", "storage"] => "--storage");
        update_cli!(opts, &mut cli_opts, ["duration"] => "--duration");
        update_targets(targets, &mut cli_opts)?;
        self.run(cli_opts)
    }
}
